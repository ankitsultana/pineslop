...
export async function GET(
  ...
  if (tableType) queryParams.set('tableType', tableType);

  const queryString = queryParams.toString();
  const url = `${clusterConfig.url}/tables/${tableName}/segmentsStatus${queryString ? `?${queryString}` : ''}`;
  ...
  try {
    ...
    console.error('Error fetching segments status from cluster:', error);
    return NextResponse.json(
      { error: 'Failed to fetch segments status from cluster' },
      { status: 500 }
    );
  }
}
