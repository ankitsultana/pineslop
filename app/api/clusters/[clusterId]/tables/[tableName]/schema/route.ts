...
export async function GET(
  ...
  // Schema name is typically the same as the table name (without _OFFLINE or _REALTIME suffix)
  const schemaName = tableName.replace(/_OFFLINE$|_REALTIME$/, '');
  const url = `${clusterConfig.url}/schemas/${encodeURIComponent(schemaName)}`;
  ...
  try {
    ...
    console.error('Error fetching schema from cluster:', error);
    return NextResponse.json(
      { error: 'Failed to fetch schema from cluster' },
      { status: 500 }
    );
  }
}
